...
export const MenuReport: React.FC<MenuReportProps> = ({ selectedBranch, dateRange }) => {
  ...
  const goodsHeaderClass = 'text-[13px] font-medium whitespace-nowrap bg-violet-50';
  const goodsBodyClass = 'text-sm text-brand-text font-medium bg-violet-50 group-hover:bg-violet-100';
  const rows = useMemo(() => {
    ...
    return MOCK_MENU_REPORT_BASE.map((row, index) => {
  ...
  const columns: ColumnDef<MenuReportRow>[] = [
    {
      ...
      headerClassName: goodsHeaderClass,
      cellClassName: goodsBodyClass,
    },
    {
      header: 'Category',
